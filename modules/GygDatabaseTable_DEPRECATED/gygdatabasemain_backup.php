...
class GygDatabase
{
	...
	public function create($table, $tableColumns, $isEnabled = true)
	{
		...
		if(count($this->select('table_settings', ['table_name' => $table])) === 0)
		{		
			$tableSettings =
			[
				'table_name' => $table,
			...
			$this->insert('table_settings', $tableSettings);
			$tableId = $this->lastInsertId();
		}
	}
	 ...
	 * @data, array, Data to be inserted. If the array lacks 
	 ...
	 * @params
	 * @table, string, ID of table
	 * @data, array, Column names and the data
	 * those columns are to be assigned.
	 * @rowAttributes, array, Optional argument. If not used,
	...
	/*
	 * Check if a table contains in the database.
	 */
